...
export function ProductActions({
  ...
  const [cta, setCta] = useState<CtaButtons | null>(null);

  useEffect(() => {
    fetch("/api/storefront/settings")
      .then((r) => r.json())
      .then((data) => {
        setCta(data.ctaButtons ? { ...defaultCta, ...data.ctaButtons } : defaultCta);
      })
      .catch(() => { setCta(defaultCta); })
  }, []);
      ...
      <div className="hidden space-y-2 md:block md:space-y-3">
        ...
        {storePhoneClean && resolvedCta.call.enabled && (
          <button
            ...
            className={`relative flex w-full items-center justify-center border-2 border-blue-600 p-4 tracking-wide text-blue-600 transition-colors hover:bg-blue-50 ${shapeClass(resolvedCta.call.style)}`}
            type="button"
          >
        ...
        {phone && resolvedCta.whatsapp.enabled && (
          <a
            ...
            className={`relative flex w-full items-center justify-center border-2 border-green-600 p-4 tracking-wide text-green-600 transition-colors hover:bg-green-50 ${shapeClass(resolvedCta.whatsapp.style)}`}
          >
        ...
        <div className="fixed inset-x-0 bottom-0 z-50 flex translate-y-0 items-center gap-2 border-t border-neutral-200 bg-white/95 px-3 py-2 shadow-[0_-8px_24px_rgba(0,0,0,0.12)] backdrop-blur supports-[padding:max(0px)]:pb-[max(0.5rem,env(safe-area-inset-bottom))] md:hidden">
          ...
          {phone && resolvedCta.whatsapp.enabled && (
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className={`flex w-full items-center justify-center gap-1.5 bg-green-600 p-3 text-sm font-medium text-white shadow-sm ${shapeClass(resolvedCta.whatsapp.style)}`}
            >
