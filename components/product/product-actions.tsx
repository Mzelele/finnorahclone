...
type CtaBtn = { enabled: boolean; text: string; style: "pill" | "rectangle" };
type CtaButtons = { addToCart: CtaBtn; call: CtaBtn; whatsapp: CtaBtn };
...
export function ProductActions({
  ...
  const [cta, setCta] = useState<CtaButtons>(defaultCta);

  useEffect(() => {
    fetch("/api/storefront/settings")
      .then((r) => r.json())
      .then((data) => {
        if (data.ctaButtons) setCta({ ...defaultCta, ...data.ctaButtons });
      })
      .catch(() => {});
  }, []);
  ...
  const renderAddToCart = (className?: string) => {
    const base = clsx(
      ...
      shapeClass(cta.addToCart.style),
      !className && "p-4",
      className
    );
    if (!availableForSale) {
    ...
    if (!selectedVariantId) {
      return (
        <button disabled className={clsx(base, disabledClasses)}>
          ...
          {cta.addToCart.text || "Add To Cart"}
        </button>
      );
    }
    return (
  ...
  return (
    <>
      ...
      <div className="hidden space-y-2 md:block md:space-y-3">
        {renderAddToCart()}
        {storePhoneClean && cta.call.enabled && (
          <button
            onClick={() => { if (showPhone) { window.location.href = telLink; } else { setShowPhone(true); } }}
            className={`relative flex w-full items-center justify-center border-2 border-blue-600 p-4 tracking-wide text-blue-600 transition-colors hover:bg-blue-50 ${shapeClass(cta.call.style)}`}
            type="button"
          >
            {showPhone ? storePhoneClean : (cta.call.text || "Call to Order")}
          </button>
        )}
        {phone && cta.whatsapp.enabled && (
          <a
            href={whatsappLink}
            target="_blank"
            rel="noopener noreferrer"
            className={`relative flex w-full items-center justify-center border-2 border-green-600 p-4 tracking-wide text-green-600 transition-colors hover:bg-green-50 ${shapeClass(cta.whatsapp.style)}`}
          >
            {cta.whatsapp.text || "WhatsApp"}
          </a>
        )}
      </div>
      ...
      {(phone || storePhoneClean) && (
        <div className="fixed inset-x-0 bottom-0 z-50 flex translate-y-0 items-center gap-2 border-t border-neutral-200 bg-white/95 px-3 py-2 shadow-[0_-8px_24px_rgba(0,0,0,0.12)] backdrop-blur supports-[padding:max(0px)]:pb-[max(0.5rem,env(safe-area-inset-bottom))] md:hidden">
          ...
          {cta.addToCart.enabled && <div className="min-w-0 flex-1 basis-0">{renderAddToCart("p-3 text-sm")}</div>}
          {phone && cta.whatsapp.enabled && (
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className={`flex w-full items-center justify-center gap-1.5 bg-green-600 p-3 text-sm font-medium text-white shadow-sm ${shapeClass(cta.whatsapp.style)}`}
            >
